  ...
  <v-container style="padding: 0px 24px 0px 24px;" fill-height>
    <v-form v-model="formValid">
      <v-layout fill-height row wrap>
        <v-flex xs8>
          <v-textarea :readonly="!editable" v-model="question.questionText" 
                      :rules="textRules" label="Question text"/>
        </v-flex>
        ...
        <v-flex v-if="question.type === 'fixed' || question.type === 'regex'" xs12>
          <v-layout row>
            ...
            <v-flex>
              <v-text-field :readonly="!editable" v-model="question.answer" :rules="answerRules" prefix/>
            </v-flex>
          </v-layout>
        </v-flex>
        <v-flex v-else-if="question.type === 'mc'" xs12>
          ...
          <v-layout column>
            <v-layout v-for="(variant, i) in question.variants" :key="`v-${i}`" 
              ...
              <v-flex xs4>
                <v-text-field :readonly="!editable" v-model="variant.choiceText" :rules="variantRules" 
                              label="Variant text" required/>
              </v-flex>
              <v-flex xs4>
...
export default {
  ...
  data() {
    return {
      ...
      variantRules: [
        v => !!v || 'Varaint text is requried'
      ]
    }
  },
  computed: {
    valid: function() {
      if (this.question.type === 'mc' && (!this.question.variants || this.question.variants.length < 1)) {
        return false;
      }
